...
const birthDate = inventors.sort((inventor) => {
...
const sum = inventors.reduce((acc, num) => {
  const born = num.passed - num.year
  console.log(born)
  return acc + born
})
...
const peopleReverse = people.map((person) => {
  const newArray = []

  const splitPerson = person.split(', ')
  const reverseArray = splitPerson.reverse()
...
const nineteenOrOlder = devs.every((dev) => {
  const today = new Date()
  const yearr = today.getFullYear()
  const devsYear = dev.year
  return yearr - devsYear > 19
})
...
const findCommentID = comments.find((cmt) => cmt.id === 823423)
...
const findIndexx = comments.findIndex((cmt) => cmt.id === 123523)
